...
# Function to clean the attributes JSON and fix improper formatting
def clean_json_field(data):
    """Clean specific fields of the data to remove extraneous characters."""
    if isinstance(data, str):
        # Remove leading 'u' and unnecessary quotes from strings (Unicode handling)
        data = re.sub(r"^u'", "", data)  # Remove "u'" from the start of the string
        data = re.sub(r"'$", "", data)  # Remove single quote from the end of the string
        data = re.sub(r"^'", "", data)  # Remove single quote from the start of the string

        # Ensure proper JSON format for strings (replace single quotes with double quotes)
        data = re.sub(r"(?<=\w)'(?!\w)", '"', data)  # Replace single quotes with double quotes for JSON keys and values
        data = re.sub(r"(False|True)", lambda x: x.group(0).lower(), data)  # Convert booleans to lowercase
        
        # Check if it's a string representation of a dictionary and convert it to valid JSON
        try:
            # Try to load it as JSON (only if it's valid)
            return json.loads(data)
        except json.JSONDecodeError:
            # Return the string if it's not valid JSON
            return data
